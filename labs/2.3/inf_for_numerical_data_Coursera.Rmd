...
ggplot(nc) + geom_boxplot(aes(x = habit, y = weight))
```

```{r}
ggplot(nc) + geom_histogram(aes(x = weight)) + facet_wrap(~habit, nrow = 3)
```
...
inference(y = weeks, data = nc, type = "ci", statistic = "mean", method = "theoretical",
          conf_level = 0.99)
```
...
```{r gained-mature-ht}
# type your code for the Exercise here, and Knit
inference(y = gained, x = mature, data = nc, type = "ht", statistic = "mean", null = 0,
          alternative = "twosided", method = "theoretical")
```
...
nc %>% group_by(mature) %>% summarise(min(mage), max(mage))
# 35
```
