...
us12 %>% group_by(response) %>% count() %>% ungroup() %>% mutate(p = n/sum(n))
```
...
inference(y = response, data = japan12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
...
inference(y = response, x = year, data = spain, statistic = "proportion", 
...
inference(y = response, x = year, data = us, statistic = "proportion", 
          type = "ht", method = "theoretical", success = "atheist",
          alternative = "twosided")
```
...
# type 1 error: h0 is true but wrongly rejected h0

# CI for total 39 countries
39 * 0.05
```
