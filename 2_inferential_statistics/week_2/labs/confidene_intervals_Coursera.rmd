...
```{r}
summary(samp$area)
# The sample mean is 1476, which is the typical size of home
# in this size 60 random sample.
```


```{r describe-sample}
ggplot(samp) + geom_histogram(aes(area))
```
...
ci %>% summarise(capture_mu_freq = sum(capture_mu == 'yes')/n())
```
